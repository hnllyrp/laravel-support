...
class GrammarMacro
{
     ...
     * 配合 BlueprintMacro::class comment 方法一起使用
     */
    public function compileTableComment(): callable
    {
        ...
        return function (Blueprint $blueprint, Fluent $command, Connection $connection) {
            switch ($connection->getDriverName()) {
                case 'mysql':
                    return sprintf('ALTER TABLE %s COMMENT = %s', $this->wrapTable($blueprint),
                        "'" . str_replace("'", "''", $command->comment) . "'");

                case 'pgsql':
                    return sprintf('comment on table %s is %s', $this->wrapTable($blueprint),
                        "'" . str_replace("'", "''", $command->comment) . "'"
                    );
